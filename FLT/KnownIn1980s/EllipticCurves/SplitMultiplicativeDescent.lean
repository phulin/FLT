...
theorem exists_isom_of_stabilizer_index_le_two (W₁ W₂ : WeierstrassCurve k) [W₁.IsElliptic]
  ...
  have hfr : Module.finrank k L ≤ 2 := by
    rw [IntermediateField.finrank_eq_fixingSubgroup_index (SeparableClosure k) L]
    exact (Subgroup.index_antitone hHle).trans hHi
...
theorem exists_isom_or_isQuadraticTwistForm_of_j_eq (W₁ W₂ : WeierstrassCurve k) [W₁.IsElliptic]
  ...
  · -- feed the obtained extension and its instances to the dichotomy explicitly (`@`), to
    ...
    rcases @exists_smul_eq_or_exists_smul_eq_quadraticTwist k _ W₁ L hF hA hQ hS _ hj₀ hj₁₇₂₈
      W₂ ⟨ρ, hρ⟩ with ⟨C, hC⟩ | ⟨C, hC⟩
