...
/-- If `r ^ N = q`, every `N`-torsion class in `G / q^ℤ` has a representative which is
an `N`-th root of unity times an integral power of `r`.  No reduction of the exponent modulo
`N` is needed: if `u ^ N = q ^ z`, then `u * (r ^ z)⁻¹` is already a root of unity. -/
theorem QuotientGroup.exists_pow_eq_one_mul_zpow_of_mk_pow_eq_one
    {G : Type*} [CommGroup G] {q r u : G} {N : ℕ} (hr : r ^ N = q)
    (hu : (u : G ⧸ Subgroup.zpowers q) ^ N = 1) :
    ∃ (ζ : G) (z : ℤ), ζ ^ N = 1 ∧ u = ζ * r ^ z := by
  have humem : u ^ N ∈ Subgroup.zpowers q := by
    rw [← QuotientGroup.eq_one_iff]
    simpa only [QuotientGroup.mk_pow] using hu
  obtain ⟨z, hz⟩ := Subgroup.mem_zpowers_iff.mp humem
  let ζ := u * (r ^ z)⁻¹
  have hrz : (r ^ z) ^ N = q ^ z := by
    rw [← zpow_natCast, zpow_comm, zpow_natCast, hr]
  refine ⟨ζ, z, ?_, ?_⟩
  · rw [show ζ = u * (r ^ z)⁻¹ from rfl, mul_pow, inv_pow, hrz, hz,
      mul_inv_cancel]
  · simp only [ζ, mul_assoc, inv_mul_cancel, mul_one]
...
-- Legacy copy retained while the surrounding PR material is migrated to the current API.
private def negVariableChange_legacy (W : WeierstrassCurve R) : VariableChange R :=
  ⟨-1, 0, -W.a₁, -W.a₃⟩
...
variables. -/
private theorem negVariableChange_mul_self_legacy (W : WeierstrassCurve R) :
...
theorem map_negVariableChange {A : Type*} [Field A] (W : WeierstrassCurve R) (φ : R →+* A) :
