...
/-- The canonical submodule generated by all differences `rho g x - x`.  Its quotient is
the largest quotient on which every `rho g` acts trivially. -/
def differenceSubmodule
    {A : Type u} {W : Type v} {G : Type*} [CommRing A]
    [AddCommGroup W] [Module A W] (rho : G → W →ₗ[A] W) : Submodule A W :=
  ⨆ g, LinearMap.range (rho g - LinearMap.id)

/-- Every generator `rho g x - x` belongs to the canonical difference submodule. -/
theorem sub_apply_mem_differenceSubmodule
    {A : Type u} {W : Type v} {G : Type*} [CommRing A]
    [AddCommGroup W] [Module A W] (rho : G → W →ₗ[A] W) (g : G) (x : W) :
    rho g x - x ∈ differenceSubmodule rho := by
  apply Submodule.mem_iSup_of_mem g
  exact ⟨x, by simp⟩

/-- A cross-annihilation identity says that the canonical difference submodule is contained
in the corresponding scalar eigenspace. -/
theorem differenceSubmodule_le_ker_of_cross_relation
    {A : Type u} {W : Type v} {G : Type*} [CommRing A]
    [AddCommGroup W] [Module A W] (rho : G → W →ₗ[A] W) (c : G → A)
    (hcross : ∀ g h,
      (rho h - c h • (LinearMap.id : W →ₗ[A] W)).comp
        (rho g - LinearMap.id) = 0) (h : G) :
    differenceSubmodule rho ≤
      LinearMap.ker (rho h - c h • (LinearMap.id : W →ₗ[A] W)) := by
  rw [differenceSubmodule]
  apply iSup_le
  intro g
  rintro _ ⟨x, rfl⟩
  change ((rho h - c h • (LinearMap.id : W →ₗ[A] W)).comp
    (rho g - LinearMap.id)) x = 0
  rw [hcross]
  rfl

/-- The cross-annihilation identity canonically produces a multiplicative submodule with
constant quotient.  This isolates the coefficient-stability step in Schoof's argument: no
chosen additive filtration has to be preserved by the coefficient ring. -/
theorem exists_multiplicative_constant_filtration_of_cross_relation
    {A : Type u} {W : Type v} {G : Type*} [CommRing A]
    [AddCommGroup W] [Module A W] (rho : G → W →ₗ[A] W) (c : G → A)
    (hcross : ∀ g h,
      (rho h - c h • (LinearMap.id : W →ₗ[A] W)).comp
        (rho g - LinearMap.id) = 0) :
    ∃ N : Submodule A W,
      (∀ g x, rho g x - x ∈ N) ∧
      (∀ g x, x ∈ N → rho g x = c g • x) := by
  refine ⟨differenceSubmodule rho,
    sub_apply_mem_differenceSubmodule (rho := rho), ?_⟩
  intro g x hx
  have hxker := differenceSubmodule_le_ker_of_cross_relation rho c hcross g hx
  simpa [LinearMap.mem_ker, sub_eq_zero] using hxker
